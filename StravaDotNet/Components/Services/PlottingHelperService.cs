...
public class PlottingHelperService
{
    ...
    /// </summary>
    public string PrepareScatterPlotData<T>(List<T> items, Func<T, float?> xSelector, Func<T, double?> ySelector, Func<T, string> colorSelector)
    {
    ...
    /// </summary>
    public string PrepareBarChartData(Dictionary<string, Dictionary<string, int>> groupedData)
    {
        var labels = groupedData.Keys.ToList();
        var datasets = groupedData.First().Value.Keys.Select(key => new
        {
            label = key,
            data = labels.Select(label => groupedData[label].ContainsKey(key) ? groupedData[label][key] : 0).ToList()
        }).ToList();
    ...
    /// </summary>
    public string GetColorForDate(DateTime? date, DateTime? minDate, DateTime? maxDate)
    {
